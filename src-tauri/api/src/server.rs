...
pub async fn start_server_with_config(
    ...
    let state = AppState {
        db,
        cache,
        user_manager: None, // Will be set by the app when available
    };
...
/// Start the API server with user manager
pub async fn start_server_with_user(
    db: Arc<database::Database>,
    cache: Arc<json_cache::CacheManager>,
    user_manager: Arc<user::UserManager>,
    config: ApiConfig,
) -> Result<(), Box<dyn std::error::Error>> {
    // Initialize test data if requested
    #[cfg(debug_assertions)]
