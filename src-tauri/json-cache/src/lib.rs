...
impl JsonCache {
    ...
    pub fn set(&self, key: &str, value: &serde_json::Value, entity_type: &str, ttl_seconds: i64, content_hash: &str) -> Result<()> {
        debug!("Caching entity: key={}, type={}, ttl={}", key, entity_type, ttl_seconds);
        ...
        debug!("Successfully cached entity: key={}, size={} bytes", key, size_bytes);
        Ok(())
    }
    ...
    pub fn get(&self, key: &str) -> Result<Option<CacheEntry>> {
        ...
        if metadata.ttl_seconds == 0 || (metadata.ttl_seconds > 0 && age_seconds > metadata.ttl_seconds) {
            debug!("Cache entry expired: key={}, age={}, ttl={}", key, age_seconds, metadata.ttl_seconds);
            return Ok(None);
        }
    ...
    pub fn stats(&self) -> Result<CacheStats> {
        ...
        for result in metadata_table.iter()? {
            ...
            *entity_types.entry(metadata.entity_type.clone()).or_insert(0) += 1;
        }
...
impl CacheManager {
    ...
    pub async fn set(&self, key: &str, value: &serde_json::Value, entity_type: &str, ttl_seconds: i64, content_hash: &str) -> Result<()> {
        let cache = self.cache.read().await;
...
mod tests {
    ...
    fn test_cache_basic_operations() {
        ...
        cache.set("test1", &value, "test_entity", 3600, "hash123").unwrap();
