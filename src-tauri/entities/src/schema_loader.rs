use crate::entity::{Entity, EntityDefinition, GenericEntity};
...
impl SchemaLoader {
    ...
    pub async fn load_entities_from_directory(dir: &Path) -> Result<Vec<Box<dyn Entity>>> {
        ...
        let entries = std::fs::read_dir(dir).map_err(|e| {
            EntitiesError::SchemaParsing(format!("Failed to read directory: {}", e))
        })?;

        for entry in entries {
            let entry = entry.map_err(|e| {
                EntitiesError::SchemaParsing(format!("Failed to read directory entry: {}", e))
            })?;
    ...
    pub fn validate_definition(definition: &EntityDefinition) -> Result<()> {
        ...
        if definition.id.is_empty() {
            return Err(EntitiesError::Validation(
...
"#;
        ...
        let entity = SchemaLoader::load_entity_from_file(&schema_path)
            .await
            .unwrap();
