...
impl DiffEngine {
            ...
            if let Some(key_str) = key.as_str() {
                if let Some(old_value) = old_map.get(key) {
                    if !Self::values_equal(old_value, new_value) {
                        ...
                        if let (Value::Mapping(old_nested), Value::Mapping(new_nested)) =
                            (old_value, new_value)
                        {
                            ...
                            diff.modified
                                .insert(full_path, Value::Mapping(change_record));
                        }
                    }
                }
            }
    ...
    fn categorize_modification(
        ...
        if (path.contains("type") && path.contains("fields")) || path.contains("cardinality") {
            ChangeCategory::Breaking
        }
    ...
    pub fn summarize_changes(changes: &[Change]) -> String {
        let safe_count = changes
            .iter()
            .filter(|c| c.category == ChangeCategory::Safe)
            .count();
        let warning_count = changes
            .iter()
            .filter(|c| c.category == ChangeCategory::Warning)
            .count();
...
mod tests {
    ...
    fn test_categorize_safe_changes() {
        ...
        diff.added.insert(
            ...
            Value::String("A description".to_string()),
        );
