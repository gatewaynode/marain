...
impl VersionTracker {
    ...
    pub async fn record_version(
        ...
        let actions_json =
            serde_json::to_string(actions).map_err(|e| sqlx::Error::Protocol(e.to_string()))?;

        // Generate rollback actions
        let rollback_actions: Vec<_> = actions.iter().filter_map(|a| a.rollback_action()).collect();

        let rollback_json = if !rollback_actions.is_empty() {
            Some(
                serde_json::to_string(&rollback_actions)
    ...
    pub async fn get_version_history(
        ...
        let mut sql =
            String::from("SELECT * FROM file_versions WHERE file_path = ? ORDER BY version DESC");
    ...
    pub async fn get_all_versions(
        ...
        let mut sql = String::from("SELECT * FROM file_versions ORDER BY applied_at DESC");
    ...
    pub async fn rollback_to_version(
        ...
        for version in versions {
            if let Some(rollback_json) = version.rollback_actions {
                match serde_json::from_str::<Vec<crate::action_generator::Action>>(&rollback_json) {
                    ...
                    Err(e) => {
                        warn!(
                            "Failed to parse rollback actions for version {}: {}",
                            version.id, e
                        );
                    }
                }
        ...
        info!(
            "Prepared {} rollback actions for file {} to version {}",
    ...
    pub async fn get_statistics(&self) -> Result<VersionStatistics, sqlx::Error> {
        ...
        let total_files: (i64,) =
            sqlx::query_as("SELECT COUNT(DISTINCT file_path) FROM file_versions")
                .fetch_one(&self.pool)
                .await?;

        let applied_versions: (i64,) =
            sqlx::query_as("SELECT COUNT(*) FROM file_versions WHERE status = 'applied'")
                .fetch_one(&self.pool)
                .await?;

        let rolled_back_versions: (i64,) =
            sqlx::query_as("SELECT COUNT(*) FROM file_versions WHERE status = 'rolled_back'")
                .fetch_one(&self.pool)
                .await?;
...
mod tests {
    ...
    async fn test_version_tracking() {
        ...
        // Get current version
        let current = tracker
            .get_current_version("schemas/test.yaml")
            .await
            .unwrap();
        ...
        let changed = tracker
            .has_file_changed("schemas/test.yaml", "xyz789")
            .await
            .unwrap();
        assert!(changed);

        let not_changed = tracker
            .has_file_changed("schemas/test.yaml", "abc123")
            .await
            .unwrap();
        assert!(!not_changed);
    }
