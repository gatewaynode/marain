...
impl Action {
    ...
    pub fn rollback_action(&self) -> Option<Action> {
        match self {
            Action::CreateTable { entity_id, table_name, .. } => {
                Some(Action::DropTable {
                    entity_id: entity_id.clone(),
                    table_name: table_name.clone(),
                })
            }
            Action::AddColumn { entity_id, table_name, column_name, .. } => {
            ...
            Action::CreateIndex { index_name, table_name, .. } => {
...
impl ActionGenerator {
                                    ...
                                    if let Some(field_type) = field_map.get(&Value::String("type".to_string())) {
                                        if field_type == &Value::String("slug".to_string()) {
                                            ...
                                            actions.push(Action::CreateIndex {
                                                index_name,
                                                table_name: table_name.clone(),
                                                columns: vec![field_id.as_str().unwrap_or("").to_string()],
                                                sql,
                                            });
                                        }
                                    }
                        ...
                        if let Some(fields_map) = added_fields.as_mapping() {
                            for (field_id, field_def) in fields_map {
                                if let Some(field_id_str) = field_id.as_str() {
                                    let sql = Self::generate_add_column_sql(&table_name, field_id_str, field_def)?;
    ...
    fn generate_create_table_sql(entity_id: &str, entity_def: &serde_yaml::Mapping) -> Result<String, String> {
        let mut columns = vec![
    ...
    fn generate_add_column_sql(table_name: &str, field_id: &str, field_def: &Value) -> Result<String, String> {
        if let Some(field_map) = field_def.as_mapping() {
            ...
            let mut column_def = format!("ALTER TABLE {} ADD COLUMN {} {}", table_name, field_id, sql_type);
            if required {
