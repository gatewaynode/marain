...
impl<'a> EntityStorage<'a> {
    ...
    pub async fn update(&self, id: &str, fields: HashMap<String, JsonValue>) -> Result<()> {
        ...
        if result.rows_affected() == 0 {
            return Err(DatabaseError::EntityNotFound(format!(
                "{} with id: {}",
                self.entity_type, id
            )));
        }
    ...
    async fn create_revision(&self, id: &str) -> Result<()> {
        ...
        let sql = format!(
            ...
            WHERE id = ?
            "#,
            revision_table, self.table_name
        );
    ...
    pub async fn get_revision(&self, id: &str, rid: i64) -> Result<Option<ContentItem>> {
        if !self.versioned {
            return Err(DatabaseError::Validation(
                "Entity is not versioned".to_string(),
            ));
        }
    ...
    pub async fn list_revisions(&self, id: &str) -> Result<Vec<i64>> {
        ...
        let rows = sqlx::query(&sql).bind(id).fetch_all(self.db.pool()).await?;
        ...
        for row in rows {
            ...
            for (i, column) in columns.iter().enumerate() {
                ...
                let value = if let Ok(v) = row.try_get::<Option<String>, _>(i) {
                    ...
                    v.map(|n| JsonValue::Number(serde_json::Number::from(n)))
...
mod tests {
    ...
    async fn test_create_and_get() {
        ...
        fields.insert(
            "title".to_string(),
            JsonValue::String("Test Title".to_string()),
        );
        fields.insert(
            "count".to_string(),
        ...
        assert_eq!(
            item.fields.get("title").unwrap(),
            &JsonValue::String("Test Title".to_string())
        );
        assert_eq!(
            item.fields.get("count").unwrap(),
            &JsonValue::Number(serde_json::Number::from(42))
        );
    }
    ...
    async fn test_delete() {
        ...
        fields.insert(
            "title".to_string(),
            JsonValue::String("To Delete".to_string()),
        );
