...
mod tests {
    ...
    fn test_validate_datetime_field() {
        ...
        assert!(FieldValidator::validate_field_value(&field, &json!("2023-01-01T00:00:00Z")).is_ok());
        assert!(FieldValidator::validate_field_value(&field, &json!("2023-01-01T00:00:00+00:00")).is_ok());
    ...
    fn test_validate_cardinality() {
        ...
        assert!(FieldValidator::validate_field_value(&single_field, &json!(["Test1", "Test2"])).is_err());
        
        // Test multi-value field with limit
        let multi_field = Field::new("tags", FieldType::Text, "Tags").with_cardinality(3);
        assert!(FieldValidator::validate_field_value(&multi_field, &json!(["Tag1", "Tag2"])).is_ok());
        assert!(FieldValidator::validate_field_value(&multi_field, &json!(["Tag1", "Tag2", "Tag3"])).is_ok());
        assert!(FieldValidator::validate_field_value(&multi_field, &json!(["Tag1", "Tag2", "Tag3", "Tag4"])).is_err());
        ...
        assert!(FieldValidator::validate_field_value(&unlimited_field, &json!(["Item1", "Item2", "Item3", "Item4", "Item5"])).is_ok());
        assert!(FieldValidator::validate_field_value(&unlimited_field, &json!("SingleItem")).is_ok());
    }
}