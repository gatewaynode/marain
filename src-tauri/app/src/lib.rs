...
use database::{initialize_database, Database, DatabaseConfig};
use json_cache::CacheManager;
use schema_manager::{config_access, get_configuration};
...
impl EnvPaths {
    ...
    pub fn from_env() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let entity_schema_path =
            std::env::var("ENTITY_SCHEMA_PATH").unwrap_or_else(|_| "./schemas".to_string());
        let configuration_path =
            std::env::var("CONFIGURATION_PATH").unwrap_or_else(|_| "./config".to_string());
...
#[tauri::command]
async fn get_snippet(state: State<'_, AppState>, id: String) -> Result<String, String> {
    use database::storage::EntityStorage;
...
#[tauri::command]
async fn cache_get(state: State<'_, AppState>, key: String) -> Result<String, String> {
...
async fn cache_set(
    ...
    match state
        .cache
        .set(&key, &value, &entity_type, ttl_seconds, &content_hash)
        .await
    {
        Ok(()) => {
...
#[tauri::command]
async fn cache_delete(state: State<'_, AppState>, key: String) -> Result<bool, String> {
...
async fn cache_stats(state: State<'_, AppState>) -> Result<String, String> {
    ...
    match state.cache.stats().await {
        Ok(stats) => {
            tracing::info!(
                "Cache stats: {} total entries, {} active",
                stats.total_entries,
...
pub fn run() {
    ...
    let env_paths = EnvPaths::from_env().expect("Failed to load environment paths");
    ...
    let _guard = logging::init_logging(&env_paths).expect("Failed to initialize logging system");
    ...
    let (db, cache) = runtime.block_on(async {
        ...
        std::env::set_var(
            "ENTITY_SCHEMA_PATH",
            env_paths_clone.entity_schema_path.to_str().unwrap(),
        );
        std::env::set_var(
            "CONFIGURATION_PATH",
            env_paths_clone.configuration_path.to_str().unwrap(),
        );
        ...
        let db_file =
            schema_manager::config_access::get_system_string("database.connections.sqlite.file")
        ...
        match initialize_database(db_config).await {
            Ok(db) => {
                ...
                if !entities.is_empty() {
                    if let Err(e) =
                        database::init::create_entity_tables_with_entities(&db, entities).await
                    {
                ...
                let cache_enabled =
                    config_access::get_system_bool("json_cache.enabled").unwrap_or(true);
                let cache = if cache_enabled {
                    ...
                    Arc::new(
                        CacheManager::new(&temp_path).expect("Failed to create disabled cache"),
                    )
                };
